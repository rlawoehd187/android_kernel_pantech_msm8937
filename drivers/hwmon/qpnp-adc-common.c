...
static const struct qpnp_vadc_map_pt pt_adcmap_btm_threshold[] = {
	{-400,	1757},
	{-390,	1754},
	{-380,	1751},
	{-370,	1747},
...
#ifdef CONFIG_PANTECH_PMIC_JEITA_SW
int32_t qpnp_pt_adc_scale_batt_therm(struct qpnp_vadc_chip *chip,
		int32_t adc_code,
		const struct qpnp_adc_properties *adc_properties,
		const struct qpnp_vadc_chan_properties *chan_properties,
		struct qpnp_vadc_result *adc_chan_result)
...
//printk("====== pt_qpnp_adc_scale_batt_therm (%lld) chan(%x)\n", bat_voltage, adc_chan_result->chan);
	return qpnp_adc_map_temp_voltage(
			pt_adcmap_btm_threshold,
			ARRAY_SIZE(pt_adcmap_btm_threshold),
			bat_voltage,
			&adc_chan_result->physical);
}
EXPORT_SYMBOL(qpnp_pt_adc_scale_batt_therm);
#endif
